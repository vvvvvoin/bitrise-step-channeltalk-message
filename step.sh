...
if [ $BITRISE_BUILD_STATUS -eq 0 ] ; then MESSAGE="✅ <b>$BITRISE_APP_TITLE</b>: build $BITRISE_BUILD_NUMBER passed! 🎉\nCommit: <code>$BITRISE_GIT_MESSAGE</code> " ; fi

RESULT=$(curl -X POST https://api.exp.channel.io/open/v5/groups/${channeltalk_chatid}/messages?botName=Bitrise \
-H "accept: application/json" \
-H "Content-Type: application/json" \
-H "x-access-key: ${channeltalk_access_key}" \
-H "x-access-secret: ${channeltalk_access_secret}" \
